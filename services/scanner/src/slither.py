...
the JSON output into structured ``Finding`` objects. Supports custom
configuration via ``SlitherConfigBuilder``.
"""
...
class SlitherRunner:
    ...
    def _parse_output(self, raw: str) -> list[Finding]:
        ...
        for det in detectors:
            severity = SEVERITY_MAP.get(
                det.get("impact", "").lower(),
                "informational",
            )
            elements = det.get("elements", [])
            # Use first element for location info
            ...
            finding = Finding(
                ...
                confidence=None,  # Slither 0.10+ does not expose confidence in JSON
            )
    ...
    def _parse_source_mapping(sm: dict[str, Any]) -> dict[str, Any]:
        """Extract contract name and line numbers from a source mapping."""
        result: dict[str, Any] = {}
        if not sm:
            return result
        ...
        if filename:
            # Slither paths are typically relative to the project root
            result["contract"] = filename

        # The `start` and `length` are character offsets, not line numbers.
        # Slither includes `lines` array when `--solc-ast` is passed.
        lines = sm.get("lines")
    ...
    def _build_recommendation(det: dict[str, Any]) -> str | None:
        """Build a recommendation string from detector metadata."""
        extra = det.get("extra", {})
        if isinstance(extra, dict):
            rec = extra.get("recommendation") or extra.get("fix")
            if rec:
                return str(rec)
        # Fall back to SWC description
    ...
    def _extract_swc(det: dict[str, Any]) -> str | None:
        """Extract SWC ID from various fields."""
        # Slither sometimes puts SWC in `extra`
        extra = det.get("extra", {})
        if isinstance(extra, dict):
            swc = extra.get("swc_id")
            if swc:
                return str(swc)
        # Check references for SWC-XXX pattern
