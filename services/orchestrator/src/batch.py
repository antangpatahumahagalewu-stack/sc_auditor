...
from src.models import AuditRecord, PipelineState, QueueItem
...
class BatchProcessor:
    ...
    def get_debounce_status(self, chain: str, address: str) -> Tuple[bool, Optional[str]]:
        ...
        for record_list in [self._pipeline.get_all_records()]:
            ...
            for r in records:
                ...
                if cid == contract_id and r.state == PipelineState.COMPLETED:
                    if r.updated_at:
                        elapsed = datetime.now(timezone.utc) - r.updated_at
    ...
    async def process_queue(self, count: Optional[int] = None) -> Dict[str, Any]:
