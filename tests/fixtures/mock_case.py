...
from src.models import Case, CaseCreate, CaseStatus, ScannerFinding, Severity
...
def make_closed_case(
    ...
    case = make_open_case(case_id, detector=detector, severity=severity)
    now = datetime.now(timezone.utc).isoformat()
    case.status = CaseStatus.CLOSED
    case.closed_at = now
    case.closed_reason = "confirmed"
    case.bounty_amount = 5000.0
    case.updated_at = now
    return case
